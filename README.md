...
- **Optional VIVERSE integration**: connect a VIVERSE account to play as its active VRM/GLB avatar with VRMA idle animation, and submit a completed run's time to a global speedrun leaderboard visible from "See records" on the start screen. Entirely optional — the game is fully playable as a guest with no account.
